...
$string = "SELECT tblDocItems.depth, tblDocItems.order, tblDocItems.di, 
					  tblDocItems.content, tblDocItems.header, tblDocRequirements.description							
				FROM tblDocItems  
				LEFT JOIN tblDocRequirements ON (tblDocItems.ri = tblDocRequirements.id)				
...
while ( $row = $items->fetch_assoc () ) {
	...
	// Get x value for later
	$pdf->SetX ( $x );
	
	// Set color of tag to RED
