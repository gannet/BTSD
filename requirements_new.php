...
tblDocRequirements.di,
tblDocRequirements.description
FROM tblRegulationsDef, tblDocRequirements
WHERE tblRegulationsDef.description = tblDocRequirements.description AND
tblDocRequirements.di = " . $_SESSION ['docID'] . ";";
...
FROM tblRegulations
WHERE tblRegulations.regulatoryBody='" . $_POST ['regBody'] . "' ORDER BY
tblRegulations.title,tblRegulations.effective;";
...
WHERE tblRegulationsContent.ri='" . $_POST ['docSelect'] . "' ORDER BY
tblRegulationsContent.id;";
	$result = $header->db->query ( $string );
	$response = array ();
	while ( $row = $result->fetch_assoc () ) {
		$response [] = $row;
...
WHERE tblRegulationsDef.rci='" . $_POST ['regContent'] . "' ORDER BY
tblRegulationsDef.id;";
	$result = $header->db->query ( $string );
	$response = array ();
	while ( $row = $result->fetch_assoc () ) {
		$response [] = $row;
...
WHERE id in(" . implode ( ",", $_POST ['selectedReq'] ) . ");";
	...
	$string = "DELETE FROM tblDocRequirements WHERE di = '{$_SESSION['docID']}' AND
NOT(description IN(" . $reqDescription . "));";
	// var_dump($string);
...
$.post('<?php echo $_SERVER["PHP_SELF"] ?>' , {regBody: $('#regBody').
val()}, function(data){
...
$.post('<?php echo $_SERVER["PHP_SELF"] ?>' , {docSelect: $('#docSelect'
).val()}, function(data){
//Clear the fourth box
$('#content li').remove();
//Clear the third box
...
$.post('<?php echo $_SERVER["PHP_SELF"] ?>' , {regContent: $(
'#regContent').val()}, function(data){
$('#content li').remove();
$.each(data, function(key, val) {
$("<li value="+val.id+">"+val.description+"</li>").appendTo(
'#content');

});
}, 'json');
});
//Before moving on to actions.php this function populates the hidden
selection box and
